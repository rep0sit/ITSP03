...
public class CipherEncryption {
	...
	public void performTest() {
		try {
			...
			System.out.println("Verschluesselte Daten: " + new String(cipherBytes));
			...
			byte[] result = decryptData(cipherBytes, secretKeyBytes, cipher.getParameters().getEncoded());
	...
	public SecretKey generateSecretKey() throws InvalidKeyException, NoSuchAlgorithmException {
	...
	public byte[] encryptData(String message, Cipher cipher) throws IllegalBlockSizeException, BadPaddingException {
		// nun werden die Daten verschluesselt
		// (update wird bei grossen Datenmengen mehrfach aufgerufen werden, kann aber
		// auch weggelassen werden, doFinal mit Argument reicht für kleine Datenmengen!)
		byte[] encData = cipher.update(message.getBytes());

		// mit doFinal abschliessen (Rest inkl. Padding ..)
		byte[] encRest = cipher.doFinal();

		// Weil wir in diesem Beispiel nichts senden oder speichern, stecken wir alle Chiffrebytes in ein einziges Bytearray
	...
	public byte[] decryptData(byte[] cipherBytes, byte[] secretKeyBytes, byte[] parameterBytes)
		...
		AlgorithmParameters algorithmParms = AlgorithmParameters.getInstance("AES");
